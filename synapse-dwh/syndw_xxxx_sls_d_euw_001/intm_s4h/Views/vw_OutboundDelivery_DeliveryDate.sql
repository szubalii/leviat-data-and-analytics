...
CalculatedDelDate_precalculation AS (
    SELECT
        ...
        [DurInDays] defines the duration but only weekdays. Hence the divide by 5 and
        then multiply by 7 to get the number of weeks */
        ,DATEADD(
            day,
            CAST((DimActualRoute.[DurInDays])/5 AS INT)*7,
            OD.[ActualGoodsMovementDate]
         ) AS [HDR_ActualGoodsMovementDate_upd]
...
SELECT
    ...
    ,CAST(
         DATEADD(day,
                  CAST(((weekday-1+[leftoverdays])/6) AS INT)*2 + [leftoverdays],
                  [HDR_ActualGoodsMovementDate_upd]
                  ) AS DATE
          ) AS [CalculatedDelDate]
FROM
