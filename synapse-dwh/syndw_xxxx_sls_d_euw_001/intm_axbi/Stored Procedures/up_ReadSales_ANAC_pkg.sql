...
BEGIN
    ...
    declare @lYear smallint = (select datepart(year,max(Accountingdate)) from [base_tx_ca_0_hlp].[CUSTINVOICETRANS_ANAK]),
			@lMonth tinyint = (select datepart(month,max(Accountingdate)) from [base_tx_ca_0_hlp].[CUSTINVOICETRANS_ANAK]),
            @lRate numeric(15,6)
    ...
    where datepart(Year, a.ACCOUNTINGDATE) = @P_Year 

	-- Calculation of Ancon Connolly Sales Data per Month
	EXEC [intm_axbi].[up_ReadSales_ANAC_Jahr_Monat] @P_Year, @P_Month
