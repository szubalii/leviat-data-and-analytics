...
WHERE
  UPPER(BDT.BillingDocumentType) NOT LIKE '%PROFORMA%'
  AND
  UPPER(BDT.BillingDocumentType) NOT LIKE '%PRO FORMA%'
  AND
  UPPER(COALESCE(BDI.ProductSurrogateKey, '')) NOT LIKE '%DUMMY%'
  AND
  BDI.CancelledInvoiceEffect = 'N'
