CREATE PROC [edw].[sp_load_fact_BillingDocumentItem_axbi]
    @t_jobId        [varchar](36),
    @t_jobDtm       [datetime],
...
BEGIN
    ...
    BEGIN TRY
	    IF OBJECT_ID('[edw].[fact_BillingDocumentItem_axbi_tmp]') IS NOT NULL
		    DROP TABLE [edw].[fact_BillingDocumentItem_axbi_tmp];

        CREATE TABLE [edw].[fact_BillingDocumentItem_axbi_tmp]
        WITH
        (
            DISTRIBUTION = HASH (BillingDocument),
            CLUSTERED COLUMNSTORE INDEX
        )
        as select * from  [edw].[fact_BillingDocumentItem_axbi] WHERE 1=0;

	    SELECT TOP 1 @isAXBIViewNotEmpty = 1 FROM [edw].[vw_BillingDocumentItem_axbi];

        IF (@isAXBIViewNotEmpty = 0)
            begin
                SET @errmessage = 'Temporary view [edw].[fact_BillingDocumentItem_axbi] was not filled with data from AXBI.';
                THROW 50001, @errmessage, 1;
            end;

        INSERT INTO [edw].[fact_BillingDocumentItem_axbi_tmp](
            [BillingDocument]
        ,   [BillingDocumentItem]
        ,   [CurrencyTypeID]
        ,   [CurrencyType]
        ...
        RENAME OBJECT [edw].[fact_BillingDocumentItem_axbi] TO [fact_BillingDocumentItem_axbi_old];
        RENAME OBJECT [edw].[fact_BillingDocumentItem_axbi_tmp] TO [fact_BillingDocumentItem_axbi];
        DROP TABLE [edw].[fact_BillingDocumentItem_axbi_old];
